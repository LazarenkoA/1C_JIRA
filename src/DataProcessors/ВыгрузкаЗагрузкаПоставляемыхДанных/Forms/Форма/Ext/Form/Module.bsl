...
&НаКлиенте
Процедура ВыгрузитьДанныеЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
  Если РезультатВопроса = КодВозвратаДиалога.Отмена Тогда
    Возврат;
  ИначеЕсли РезультатВопроса = КодВозвратаДиалога.Да ИЛИ РезультатВопроса = КодВозвратаДиалога.Таймаут Тогда
    //Парам = Новый Структура("ОсуществляемыеПроверки", "ПроверятьПоддержку");
    ОткрытьФорму("ОбщаяФорма.БФТ_НастройкиОбновленияФайловШаблонов");  
  Иначе
    ВыгрузитьДанныеНаКлиенте();  
  КонецЕсли;
КонецПроцедуры
