Функция НайтиСоздатьПользователя(ДанныеПользователя) Экспорт 
	...
	Попытка
		...
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.БлокировкаПользователей");
		ЭлементБлокировки.УстановитьЗначение("НаименованиеПользователя", ДанныеПользователя["name"]);
		...
		Если ДанныеАватар <> Неопределено Тогда
			...
			Аватар = ВзаимодействиеC_JIRA_Сервер.ВыполнитьGETЗапрос(СтрЗаменить(СсылкаАватар, "mm&s=48", "mm&s=128"), Истина);
		КонецЕсли;
