...
Функция СформироватьКомандуВыполнения(МетодСсылка, ВнешниеПараметры = Неопределено, ВызыватьИсключение = Истина) Экспорт 
  ...
  Шаблон = МетодСсылка.Шаблон;	
	Для Каждого Параметр Из МетодСсылка.ПараметрыВыполнения Цикл
		
		Если МетодСсылка.СерверныеНастройки ИЛИ Параметр.РежимВводаПараметра = Перечисления.БФТ_РежимВводаПараметра.ВыполнитьМетод
			ИЛИ Параметр.РежимВводаПараметра = Перечисления.БФТ_РежимВводаПараметра.ВнешнийПараметр Тогда
			ЗначениеПараметра = Параметр.ЗначениеПараметра;
		Иначе
			ЗначениеПараметра = ХранилищеОбщихНастроек.Загрузить(Строка(МетодСсылка.УникальныйИдентификатор()) , Строка(Параметр.ПорядковыйНомерПараметра));
		КонецЕсли;
    
    Если Параметр.РежимВводаПараметра = Перечисления.БФТ_РежимВводаПараметра.ВыполнитьМетод 
      И ЗначениеЗаполнено(Параметр.ЗначениеПараметра) Тогда
      ЗначениеПараметра = Вычислить(СтрШаблон("%1()", Параметр.ЗначениеПараметра));
		ИначеЕсли Параметр.РежимВводаПараметра = Перечисления.БФТ_РежимВводаПараметра.ЗначениеКонстанты Тогда
			ЗначениеПараметра = Константы[Параметр.ЗначениеПараметра].Получить();
    ИначеЕсли Параметр.РежимВводаПараметра = Перечисления.БФТ_РежимВводаПараметра.ВнешнийПараметр Тогда
      Если ВнешниеПараметры = Неопределено ИЛИ Не ВнешниеПараметры.Свойство(Параметр.ЗначениеПараметра, ЗначениеПараметра) Тогда
        ЗначениеПараметра = "";
      КонецЕсли;
    КонецЕсли;                         
    
    Шаблон = СтрЗаменить(Шаблон, СтрШаблон("{%1}", Параметр.ПорядковыйНомерПараметра), ЗначениеПараметра);
	КонецЦикла;
	Если ВызыватьИсключение И МетодСсылка.ПараметрыВыполнения.Количество() = 0 И СтрЧислоВхождений(Шаблон, "{") > 0 Тогда
		ВызватьИсключение СтрШаблон("Не настроен метод ""%1""", МетодСсылка);
	КонецЕсли;
  
  Возврат СтрШаблон("""%1"" %2", МетодСсылка.ИсполняемыйФайл, Шаблон);
КонецФункции
...
Функция ПолучитьНомерПоследнейРевизии() Экспорт  
  ...
  Запрос.Текст = 
    ...
    |ГДЕ
    |	НЕ БФТ_ИзмененияРепозиторияСрезПоследних.ИзменениеКонфигурации
    |
    |УПОРЯДОЧИТЬ ПО
    |	НомерРевизии УБЫВ";
  
  Выборка = Запрос.Выполнить().Выбрать();
  Возврат Формат(?(Выборка.Следующий(), Выборка.НомерРевизии, 0), "ЧН=0; ЧГ=");
КонецФункции
