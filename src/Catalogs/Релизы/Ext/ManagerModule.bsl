...
Процедура ПереместитьВВерсию(ВыбранныеЗадачи, НоваяВерсия, БезПроверки = Ложь) Экспорт 
...
Функция ТекущаяВерсияРазработка() Экспорт 
...
Функция ТекущаяВерсияТестирования() Экспорт 
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	Релизы.Ссылка КАК Ссылка
