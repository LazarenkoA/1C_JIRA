	...
	Для Каждого Задача Из ВыбранныеЗадачи Цикл 
		...
		Попытка
			...
			Для Каждого Сабтаск Из Сабтаски Цикл
				ДанныеВерсий = Новый Структура("fixVersions");
				ДанныеВерсий.fixVersions = КонвертироватьВерсииВJSON(Сабтаск.Версии.ВыгрузитьКолонку("Версия"));
				ВзаимодействиеC_JIRA_КлиентСервер.ИзменитьПоляЗадачи(Сабтаск.Код, ДанныеВерсий);
			КонецЦикла;
...
Функция ПолучитьСабтаски(ЗадачаРодитель)
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
