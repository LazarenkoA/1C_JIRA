...
Процедура ИнициализироватьШагиСборки()
	...
	НовСтр = СтрокаЛогаРодитель.ПолучитьЭлементы().Добавить();
	НовСтр.Комментарий = "Актуализация настроек синхронизатора (тексты шаблонов)";
	НовСтр.Метод = СтрШаблон("АктуализацияНастроекСинхронизатора_%1", "ФункцииРасширения");                  
	НовСтр.Картинка = 0;
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаЛогаРодитель = Задания.ПолучитьЭлементы().Добавить();
		СтрокаЛогаРодитель.Комментарий = СтрШаблон("Обновление базы (%1)", Выборка.СтрокаПодключения);
		СтрокаЛогаРодитель.АдресХранилища = ПоместитьВоВременноеХранилище(Неопределено, ЭтаФорма.УникальныйИдентификатор);
		СтрокаЛогаРодитель.Картинка = 0;
		СтрокаЛогаРодитель.Метод = "ОбновитьБазу";
		СтрокаЛогаРодитель.ДопПараметр = Выборка.СтрокаПодключения;
...
Процедура ОбновлениеВерсииАЦКБУНаСервере()
	...
	ПутьКРасширению = СтрШаблон("%1\%2\CommonModules\БФТ_ОбновлениеИнформационнойБазы\Ext\Module.bsl", Константы.ПутьККаталогуSVN.Получить(), ПутьККаталогуРасширения);
...
Процедура ПолучитьРасширениеИзВременногоХранилища(АдресХранилища, ДопПараметр)
	ДД = ПолучитьИзВременногоХранилища(АдресХранилища);
	Если ДД <> Неопределено Тогда
		ДД.Записать(ПутьККаталогуСРелизом + СтрШаблон("\%1 (%2).cfe", Объект.Наименование, ДопПараметр));
	КонецЕсли;
...
Процедура ПолучитьФайлНастроекИзВременногоХранилища(АдресХранилища, ДопПараметр)
	ДД = ПолучитьИзВременногоХранилища(АдресХранилища);
	Если ДД <> Неопределено Тогда
		ДД.Записать(ПутьККаталогуСРелизом + СтрШаблон("\%1.xml", Объект.Наименование));
	КонецЕсли;

	//Если ПолучитьИзВременногоХранилища(АдресХранилища) <> Неопределено Тогда
...
&НаКлиенте
Процедура БазыДляАвтоОбновленияПослеУдаления(Элемент)
	ИнициализироватьШаги();
КонецПроцедуры
