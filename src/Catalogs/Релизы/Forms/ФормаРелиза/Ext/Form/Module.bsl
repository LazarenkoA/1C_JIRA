...
Процедура ОбновитьБазу(Строка)
	Попытка
		ОбновитьБазуНаСервере(Строка.ДопПараметр, Строка.АдресХранилища, Строка.ИдентификаторЗадания);
		ЭтаФорма.ПодключитьОбработчикОжидания("ПроверкаВыполненияЗаданий", 1, Ложь);	
	Исключение
		Строка.Картинка = 3;
...
&НаСервере
Процедура ОбновитьБазуНаСервере(СтрокаПодключения, АдресХранилища, ИдентификаторЗадания);
	Парам = Новый Массив(); 
	Парам.Добавить(АдресХранилища);
	Парам.Добавить(СтрокаПодключения);
	
	АдресаХранилищФайлов = Новый Массив();
	Для Каждого Файлы Из НайтиФайлы(ПутьККаталогуСРелизом, "*.*") Цикл
		//Адрес = ПоместитьФайлы(
	КонецЦикла;
	Парам.Добавить(АдресаХранилищФайлов);
