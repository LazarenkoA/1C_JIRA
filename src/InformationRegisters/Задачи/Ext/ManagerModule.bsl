...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Процедура ОбновитьЗадачи(НомераДдяОбновления = Неопределено, Знач Часы = 1) Экспорт 
		...
		Обсуждение.Участники.Добавить(СистемаВзаимодействия.ПолучитьИдентификаторПользователя(ИдентификатоПользователяИБ)); 
		Обсуждение.Записать();
		...
		JQL = СтрШаблон("(updated >= -%1h %2 ) and %3", Формат(Часы, "ЧГ="), ФильтрПоДатаСоздания, JQL);
		СписокЗадач = ВзаимодействиеC_JIRA_КлиентСервер.ПолучитьСписокЗадач(JQL);
