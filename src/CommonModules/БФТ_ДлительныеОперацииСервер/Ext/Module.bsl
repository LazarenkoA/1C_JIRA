...
Процедура СборкаРасширений(ИмяРасширения, АдресХранилищаРезультата, ПутьККаталогуСФайламиРасширения) Экспорт 
...
Процедура ПолучитьФайлРасширений(СтрокаПодключенияКВременнойБД, АдресХранилищаРезультата, ПутьККаталогуСФайламиРасширения, ИмяРасширения)
	Перем КодВозврата;
	...
	Если КодВозврата = 0 Тогда                                                   
		...
		Если КодВозврата = 0 Тогда 
			...
			Если БФТ_ФайлСуществует(ПутьКСохраняемойКонфигурации) Тогда		
				...
				ПоместитьВоВременноеХранилище(ДвоичныеДанные, АдресХранилищаРезультата);
				ОбщегоНазначенияКлиентСервер.УдалитьФайлыКлиентСервер(ПутьКСохраняемойКонфигурации);
			КонецЕсли;
		Иначе
			ВызватьИсключение СтрШаблон("Произошла ошибка при выгрузки расширения:
			|%1", ПолучитьЛогВыгрузкиФайловКонфигурации(ФайлЛога));
		КонецЕсли;
	Иначе
		ВызватьИсключение СтрШаблон("Произошла ошибка при загрузки файлов расширения:
		|%1", ПолучитьЛогВыгрузкиФайловКонфигурации(ФайлЛога));
	КонецЕсли;
	
КонецПроцедуры

Процедура ВыгрузитьКонфигурациюИзХранилища(АдресХранилищаРезультата, ПутьКХранилищу, ПользовательХранилища, ПарольХранилища, Ревизия,
	ПутьККаталогуСФайламиРасширения, ПутьККаталогуШаблонов, ИмяРасширения) Экспорт
  Перем ПутьКВременнойБД, АдресХранилища;
  ...
  ДанныеДляОсновногоПотока = Новый Структура("ТекущийШаг, ВыполненныеШаги", "", Новый Массив());
	...
	Попытка
		...
		АктуализацияНастроекСинхронизатора(ПутьКВременнойБД, ПутьККаталогуШаблонов, ПутьККаталогуСФайламиРасширения, ИмяРасширения);
...
#Область ВыгрузитьКонфигурациюИРасширенияИзБазы

Процедура ВыгрузитьКонфигурациюИзБазы(АдресХранилищаРезультата, СтрокаПодключенияКБазе, Пользователь, Пароль, ИмяРасширения) Экспорт 
	//ДанныеДляОсновногоПотока = Новый Структура("ТекущийШаг, ВыполненныеШаги", "", Новый Массив());
...
Процедура ВыгрузитьРасширения(АдресХранилищаРезультата, СтрокаПодключенияКБазе, Пользователь, Пароль, ИмяРасширения)
	...
	Если КодВозврата = 0 Тогда
		...
		Если БФТ_ФайлСуществует(ПутьКРасширению) Тогда		
			...
			ОбщегоНазначенияКлиентСервер.УдалитьФайлыКлиентСервер(ПутьКРасширению);
		КонецЕсли;
	Иначе
		ВызватьИсключение СтрШаблон("Произошла ошибка при выгрузки файлов расширения:
		|%1", ПолучитьЛогВыгрузкиФайловКонфигурации(ФайлЛога));
	КонецЕсли;
КонецПроцедуры

Процедура ВыгрузитьКонфигурацию(АдресХранилищаРезультата, СтрокаПодключенияКБазе, Пользователь, Пароль)
	Перем КодВозврата;
...
#Область ЗагрузитьКонфигурациюИРасширенияИзБазы

Процедура ОбновитьБазу(СтрокаПодключенияКБД, ПутьКАрхиву) Экспорт 
	ДанныеДляОсновногоПотока = Новый Структура("ТекущийШаг, ВыполненныеШаги", "", Новый Массив());
	...
	Попытка
		...
		ВремТабл.Сортировать("Очередность Возр");
		Для Каждого Стр Из ВремТабл Цикл
			Если ЗначениеЗаполнено(Стр.ИмяРасширения) Тогда
				ДанныеДляОсновногоПотока.ТекущийШаг = "ОбновлениеРасширений";
			Иначе
				ДанныеДляОсновногоПотока.ТекущийШаг = "ОбновленияКонфигурации";	
			КонецЕсли;
			ОтправитьДанныеФЗ(ДанныеДляОсновногоПотока);
			
			ЗагрузитьФайлВБазу(Стр.ПутьКФайлу, Стр.ИмяРасширения, СтрокаПодключенияКБД);
			
			ДанныеДляОсновногоПотока.ВыполненныеШаги.Добавить(ДанныеДляОсновногоПотока.ТекущийШаг);
			ОтправитьДанныеФЗ(ДанныеДляОсновногоПотока);
		КонецЦикла;
		
		ОбщегоНазначенияКлиентСервер.УдалитьФайлыКлиентСервер(ВремКаталог);
		ОбщегоНазначенияКлиентСервер.УдалитьФайлыКлиентСервер(ПутьКАрхиву);
	Исключение
		ОбщегоНазначенияКлиентСервер.УдалитьФайлыКлиентСервер(ВремКаталог);
		ОбщегоНазначенияКлиентСервер.УдалитьФайлыКлиентСервер(ПутьКАрхиву);
		ВызватьИсключение;
...
Процедура ЗагрузитьФайлВБазу(ИмяФайла, ИмяРасширения, Знач СтрокаПодключенияКБД)
	СтрокаЗапускаПлатформы = КаталогПрограммы() + "1cv8.exe";
	ФайлЛога = ПолучитьИмяВременногоФайла();                 
...
Процедура ПолучитьЛогинПароль(СтрокаПодключения, ИмяПользователя, Пароль)
	Пароль = "";
	ИмяПользователя = "";
...
Процедура СохранениеНастроек(АдресХранилищаРезультата, СтрокаПодключенияКБазе, Пользователь, Пароль) Экспорт 
	Перем КодВозврата;
	...
	СтрокаЗапуска = СтрШаблон("""%1"" ENTERPRISE /C ""РежимОтладки,ВыгрузитьИзмененныеМакеты," +
									  "ФайлДляСохранения{%2}
										|ФайлРезультат{%3},
										|ФайлЛог{%4}""" +
										" /S ""%5"" /N ""%6"" /P ""%7"" /OUT ""%8""",
	...
	ЗапуститьПриложение(СтрокаЗапуска,, Истина, КодВозврата);
	КодВозврата = ?(КодВозврата = 0, ПолучитьКодВозврата(ФайлРезультат), КодВозврата);
	
	Если КодВозврата = 0 Тогда
		Если БФТ_ФайлСуществует(ФайлЛога) Тогда
			ОбщегоНазначенияКлиентСервер.УдалитьФайлыКлиентСервер(ФайлЛога);   
		КонецЕсли;
		Если БФТ_ФайлСуществует(ФайлЛогВыполнения) Тогда
				ОбщегоНазначенияКлиентСервер.УдалитьФайлыКлиентСервер(ФайлЛогВыполнения);   
		КонецЕсли;
		Если БФТ_ФайлСуществует(ПутьКНастройкам) Тогда		
			ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ПутьКНастройкам), АдресХранилищаРезультата);
			ОбщегоНазначенияКлиентСервер.УдалитьФайлыКлиентСервер(ПутьКНастройкам);
		КонецЕсли;
	Иначе
		ВызватьИсключение СтрШаблон("Произошла ошибка при выгрузки настроек:
		|%1", ПолучитьЛогВыгрузкиФайловКонфигурации(ФайлЛога, ФайлЛогВыполнения));
	КонецЕсли;
...
Процедура ОбновитьДанныеЗадачВФоне(НомераЗадач) Экспорт 
...
Процедура АктуализацияНастроекСинхронизатора(ПутьКВременнойБД, ПутьККаталогуШаблонов, ПутьККаталогуСФайламиРасширения, ИмяРасширенияФункции)
	ФайлЛога = ПолучитьИмяВременногоФайла(); 
	ФайлРезультат = ПолучитьИмяВременногоФайла(); 
	ФайлЛогВыполнения = ПолучитьИмяВременногоФайла(); 
	
	ИмяРасширенияТиповые= "ТиповыеЦПК";
	...
	ПутьККаталогуСФайламиРасширенияТиповые = СтрШаблон("%1\%2", ПутьККаталогуСФайламиРасширения, ИмяРасширенияТиповые);
	...
	КонецЕсли;
	Если Не БФТ_ФайлСуществует(ПутьККаталогуСФайламиРасширенияТиповые) Тогда
		ВызватьИсключение СтрШаблон("Каталог ""%1""  не найден", ПутьККаталогуСФайламиРасширенияТиповые);
	КонецЕсли;
	...
	СтрокаЗапускаПлатформы = КаталогПрограммы() + "1cv8.exe";
	//СтрокаЗапуска = """%1"" DESIGNER /DisableStartupMessages" +
	//"/F ""%2"" /LoadConfigFromFiles ""%3"" -Extension %4 /UpdateDBCfg /OUT ""%5""";
	//СтрокаЗапуска = СтрШаблон(СтрокаЗапуска, СтрокаЗапускаПлатформы,
	//ПутьКВременнойБД,
	//ПутьККаталогуСФайламиРасширения,
	//ИмяРасширенияТиповые,
	//ФайлЛога);
	//
	//ЗапуститьПриложение(СтрокаЗапуска,, Истина, КодВозврата);
	//Если КодВозврата <> 0 Тогда
	//	ВызватьИсключение СтрШаблон("Произошла ошибка при загрузки расширения ""%1"":
	//	|%2", ИмяРасширенияТиповые, ПолучитьЛогВыгрузкиФайловКонфигурации(ФайлЛога));	
	//КонецЕсли;
	
	ЗапуститьПриложение(СтрШаблон("""%1"" ENTERPRISE /C""РежимОтладки,АктуализацияНастроекСинхронизатора,
	|ФайлРезультат{%6},
	|ФайлЛог{%7},
	|ПутьКИсходнымНастройкамСинхронизатора{%3},
	|ПутьКШаблонам{%4},
	|ПутьКРезультирующимНастройкамСинхронизатора{%5}"" /F""%2"" /OUT ""%8""",
	СтрокаЗапускаПлатформы,
	ПутьКВременнойБД, 
	ПутьКФайлуНастроекСинхронизатора, 
	ПутьККаталогуШаблонов, 
	ПутьКФайлуНастроекСинхронизатора,
	ФайлРезультат,
	ФайлЛогВыполнения,
	ФайлЛога
	...
	//ДД = ПолучитьИзВременногоХранилища(ФайлРезультатАдрес);
...
Процедура ПоместитьНастройкиВSVN() 
  ...
  СтрокаЗапуска = """%1"" DESIGNER /DisableStartupDialogs /DisableStartupMessages /F ""%2"" /ConfigurationRepositoryF ""%3""" +
	...
	Если КодВозврата = 0 Тогда 
		Если БФТ_ФайлСуществует(ПутьКСохраняемойКонфигурации) Тогда
			...
			ЗапуститьПриложение(СтрШаблон("""%1"" DESIGNER /F""%2"" /LoadCfg  ""%3"" /UpdateDBCfg /OUT ""%4""", СтрокаЗапускаПлатформы, СтрокаПодключенияКВременнойБД, ПутьКСохраняемойКонфигурации, ФайлЛога), , Истина, КодВозврата);
			Если КодВозврата <> 0 Тогда
				 ВызватьИсключение СтрШаблон("При загрузке конфигурации хранилища произошла ошибка:
   					 													|%1", ПолучитьЛогВыгрузкиФайловКонфигурации(ФайлЛога));
			КонецЕсли;
...
Функция ПолучитьЛогВыгрузкиФайловКонфигурации(ФайлЛога, ДопФайлЛога = Неопределено)
...
Функция ПолучитьКодВозврата(ФайлРезультат)
  Если Не БФТ_ФайлСуществует(ФайлРезультат) Тогда
    Возврат 0;  
  КонецЕсли;
