...
Процедура АктуализацияНастроекСинхронизатора(ПутьКВременнойБД, ПутьККаталогуШаблонов, ПутьККаталогуСФайламиРасширения, ИмяРасширенияФункции)
	...
	КонецЕсли;
	Если Не БФТ_ФайлСуществует(ПутьККаталогуСФайламиРасширенияВерсия) Тогда
		ВызватьИсключение СтрШаблон("Каталог ""%1""  не найден", ПутьККаталогуСФайламиРасширенияВерсия);
	КонецЕсли;
	...
	Если Данные = Неопределено Тогда
		ВызватьИсключение СтрШаблон("Не удалось получить файл расширений ""%1""", ИмяРасширения);	
	КонецЕсли;
	Данные.Записать(ФайлРасширения);
	
	
	#Область ЗагрузкаРасширения
	Параметры = Новый Структура();
	Параметры.Вставить("Расширения", ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Новый Структура("Имя, Файл", ИмяРасширения, ФайлРасширения)));
	Параметры.Вставить("ФайлРезультат", ФайлРезультат);
	Параметры.Вставить("ФайлЛог", ФайлЛогВыполнения);
	...
	КодВозврата = 0;	
	СтрокаЗапускаПлатформы = КаталогПрограммы() + "1cv8.exe";
	СтрокаЗапуска = СтрШаблон("""%1"" ENTERPRISE /C""РежимОтладки,ЗагрузкаРасширения=(%2)"" /F""%3"" /OUT ""%4""",
	...
	КодВозврата = 0;	
	СтрокаЗапускаПлатформы = КаталогПрограммы() + "1cv8.exe";
	СтрокаЗапуска = СтрШаблон("""%1"" ENTERPRISE /C""РежимОтладки,АктуализацияНастроекСинхронизатора=(%2)"" /F""%3"" /OUT ""%4""",
												СтрокаЗапускаПлатформы,
												ТелоJSON,
												ПутьКВременнойБД,
												ФайлЛога);
