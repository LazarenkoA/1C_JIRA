...
Процедура ВыгрузитьКонфигурациюИзХранилища(АдресХранилищаРезультата, ПутьКХранилищу, ПользовательХранилища, ПарольХранилища, Ревизия,
  ...
  ДанныеДляОсновногоПотока = Новый Структура("ТекущийШаг, ВыполненныеШаги", "", Новый Массив());
	...
	Попытка
		...
		////////////////////////////////////////////////////////////////////////////////////////
		ДанныеДляОсновногоПотока.ТекущийШаг = СтрШаблон("ОбновлениеШаблоновРасширенияДоп_%1", ИмяРасширения);
		ОтправитьДанныеФЗ(ДанныеДляОсновногоПотока);
		
		ПутьКФайлуНастроекСинхронизатора = СтрШаблон("%1\%2\DataProcessors\БФТ_ВыгрузкаЗагрузкаПоставляемыхДанных\Templates\НастройкиСинхронизатора\Ext\Template.txt", ПутьККаталогуСФайламиРасширения, ИмяРасширения);
		...
		ЗапуститьПриложение(СтрШаблон("""%1"" ENTERPRISE /C""РежимОтладки;АктуализацияНастроекСинхронизатора,
									  ...
									  |ПутьКШаблонам{%4},
									  |ПутьКРезультирующимНастройкамСинхронизатора{%5}"" /F""%2""",
									  КаталогПрограммы() + "1cv8.exe",
									  ПутьКВременнойБД, 
									  ПутьКФайлуНастроекСинхронизатора, 
									  ПутьККаталогуШаблонов, 
									  ПутьКФайлуНастроекСинхронизатора), , Истина);
  ...
  СтрокаЗапуска = """%1"" DESIGNER /DisableStartupDialogs /DisableStartupMessages /F ""%2"" /ConfigurationRepositoryF ""%3""" +
	...
	Если КодВозврата = 0 Тогда 
		Если БФТ_ФайлСуществует(ПутьКСохраняемойКонфигурации) Тогда
			...
			//+++Д Ганеев 13.04.2018 !dlHhsq!+
			ЗапуститьПриложение(СтрШаблон("""%1"" DESIGNER /F""%2"" /LoadCfg ""%3"" /OUT ""%4""", СтрокаЗапускаПлатформы, СтрокаПодключенияКВременнойБД, ПутьКСохраняемойКонфигурации, ФайлЛога), , Истина, КодВозврата);
			Если КодВозврата <> 0 Тогда
				 ВызватьИсключение СтрШаблон("При загрузке конфигурации хранилища произошла ошибка:
   					 													|%1", ПолучитьЛогВыгрузкиФайловКонфигурации(ФайлЛога));
			КонецЕсли;
			ЗапуститьПриложение(СтрШаблон("""%1"" DESIGNER /F ""%2"" /UpdateDBCfg /OUT ""%3""", СтрокаЗапускаПлатформы, СтрокаПодключенияКВременнойБД, ФайлЛога), , Истина, КодВозврата);
			Если КодВозврата <> 0 Тогда
				ВызватьИсключение СтрШаблон("При обновлении конфигурации БД на конфигурацию хранилища произошла ошибка:
   					 													|%1", ПолучитьЛогВыгрузкиФайловКонфигурации(ФайлЛога));
			КонецЕсли;			
			//+++К Ганеев 13.04.2018 !dlHhsq!-
