  ...
  СтрокаЗапуска = """%1"" DESIGNER /DisableStartupDialogs /DisableStartupMessages /F ""%2"" /ConfigurationRepositoryF ""%3""" +
	...
	Если КодВозврата = 0 Тогда 
		...
		Если БФТ_ФайлСуществует(ПутьКСохраняемойКонфигурации) Тогда
			...
			//+++Д Ганеев 13.04.2018 !dlHhsq!+
			ЗапуститьПриложение(СтрШаблон("""%1"" DESIGNER /F""%2"" /LoadCfg""%3""", СтрокаЗапускаПлатформы, СтрокаПодключенияКВременнойБД, ПутьКСохраняемойКонфигурации), , Истина, КодВозврата);
			Если КодВозврата <> 0 Тогда
				ВызватьИсключение("При загрузке конфигурации хранилища произошла ошибка");
			КонецЕсли;
			ЗапуститьПриложение(СтрШаблон("""%1"" DESIGNER /F""%2"" /UpdateDBCfg", СтрокаЗапускаПлатформы, СтрокаПодключенияКВременнойБД), , Истина, КодВозврата);
			Если КодВозврата <> 0 Тогда
				ВызватьИсключение("При обновлении конфигурации БД на конфигурацию хранилища произошла ошибка");
			КонецЕсли;			
			//+++К Ганеев 13.04.2018 !dlHhsq!-
