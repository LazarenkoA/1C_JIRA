...
Процедура СборкаРасширений(ИмяРасширения, АдресХранилищаРезультата, ПутьККаталогуСФайламиРасширения) Экспорт 
  ...
  ДанныеДляОсновногоПотока = Новый Структура("ТекущийШаг, ВыполненныеШаги", "", Новый Массив());
	...
	Попытка
		...
		ОтправитьДанныеФЗ(ДанныеДляОсновногоПотока);
		ПолучитьФайлРасширений(ПутьКВременнойБД, АдресХранилищаРезультата, ПутьККаталогуСФайламиРасширения);
...
Процедура ПолучитьФайлРасширений(СтрокаПодключенияКВременнойБД, АдресХранилищаРезультата, ПутьККаталогуСФайламиРасширения)
	Перем КодВозврата;
	
	СтрокаЗапускаПлатформы = КаталогПрограммы() + "1cv8.exe";
	ФайлЛога = ПолучитьИмяВременногоФайла();                 
	ПутьКСохраняемойКонфигурации = ПолучитьИмяВременногоФайла("cfe");
	ИмяРасширения = "ФункцииРасширения"; 
