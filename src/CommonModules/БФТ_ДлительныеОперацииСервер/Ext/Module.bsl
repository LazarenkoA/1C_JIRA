...
Процедура ВыполнитьПроцессПереноса(АдресХранилищаРезультата, НомерРевизии, 
  ...
  ДанныеДляОсновногоПотока = Новый Структура("ТекущийШаг, ВыполненныеШаги", "", Новый Массив());
	...
	Попытка
		...
		ПолучитьКонфигурациюИзХранилища(АдресХранилищаРезультата, ПутьКВременнойБД, ПутьКХранилищу, ПользовательХранилищаАдмин, ПарольХранилища, НомерРевизии, ДанныеДляОсновногоПотока, ВыгружатьЗащищеннуюКонфигурацию, ПутьКОбфускатору);
