...
//
Функция СтрокаВДату(Знач ДатаСтрока, Знач Время = Неопределено) Экспорт
	...
	Если Время <> Неопределено Тогда
		Разбивка = СтрРазделить(Время, ":");
		Для а = 0 По Разбивка.ВГраница() Цикл
			Если СтрДлина(Разбивка[а]) < 2 Тогда 
				Разбивка[а] = "0" + Разбивка[а];
			КонецЕсли;
		КонецЦикла;
		
		ДатаСтрока = ДатаСтрока + СтрСоединить(Разбивка, "");
	КонецЕсли;
	
	ОписаниеТипа = Новый ОписаниеТипов("Дата");
	Возврат ОписаниеТипа.ПривестиЗначение(ДатаСтрока);
КонецФункции 
