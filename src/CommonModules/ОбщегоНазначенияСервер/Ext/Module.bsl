...
Функция ИзменитьКодировку(ИсходнаяСтрока, КодировкаТекстаИсходная, КодировкаТекстаКонечная) Экспорт 
	Текст = Новый ТекстовыйДокумент;
	Текст.УстановитьТекст(ИсходнаяСтрока);
	ВФайл = ПолучитьИмяВременногоФайла();
	
	Попытка
		Текст.Записать(ВФайл, КодировкаТекстаИсходная);
		Текст.Прочитать(ВФайл, КодировкаТекстаКонечная);	
		Содержимое = Текст.ПолучитьТекст();
		
		ОбщегоНазначенияКлиентСервер.УдалитьФайлыКлиентСервер(ВФайл);
	Исключение
		ОбщегоНазначенияКлиентСервер.УдалитьФайлыКлиентСервер(ВФайл);
		ВызватьИсключение;
	КонецПопытки;
...
Функция ПолучитьПревышениеПоВерсиям() Экспорт 
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	|	СписаниеВремени.Автор.РФК КАК АвторРФК,
	|	Релизы.Ссылка КАК Версия
	|ПОМЕСТИТЬ СписаниеРазработка
	|ИЗ
	...
	|	И НЕ Релизы.РелизСобран
	|
	|СГРУППИРОВАТЬ ПО
	|	Задачи.Ссылка,
	|	Задачи.Бюджет,
	|	СписаниеВремени.Автор.РФК,
	|	Релизы.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОграничениеПоБюджетам.Бюджет КАК Бюджет,
	|	МАКСИМУМ(ОграничениеПоБюджетам.КоличествоЗаложенныхЧасов) КАК ЗаложеноВБюджет,
	|	СУММА(Задачи.Часы) КАК ФактЧасов,
	|	ОграничениеПоБюджетам.Владелец КАК Версия,
	|	Задачи.АвторРФК КАК РФК
	|ИЗ
	|	СписаниеРазработка КАК Задачи
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ОграничениеПоБюджетам КАК ОграничениеПоБюджетам
	|		ПО Задачи.Бюджет = ОграничениеПоБюджетам.Бюджет
	|			И Задачи.Версия = ОграничениеПоБюджетам.Владелец
	|			И (Задачи.АвторРФК = ОграничениеПоБюджетам.РФК
	|				ИЛИ ОграничениеПоБюджетам.РФК = ЗНАЧЕНИЕ(Справочник.РФК.ПустаяСсылка))
	|
	|СГРУППИРОВАТЬ ПО
	|	ОграничениеПоБюджетам.Владелец,
	|	Задачи.АвторРФК,
	|	ОграничениеПоБюджетам.Бюджет
	|
	|ИМЕЮЩИЕ
	|	СУММА(Задачи.Часы) > МАКСИМУМ(ОграничениеПоБюджетам.КоличествоЗаложенныхЧасов)";
...
Функция ПолучитьРасхождениеВОценках() Экспорт 
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |			И ((ВЫРАЗИТЬ(Сабтаски.Оценка / Задачи.Оценка КАК ЧИСЛО(2, 1))) > &КооэфРазработка
	               |					И Сабтаски.Тип В ИЕРАРХИИ (ЗНАЧЕНИЕ(Справочник.ТипыЗадач.Разработка))
	               |				ИЛИ (ВЫРАЗИТЬ(Сабтаски.Оценка / Задачи.Оценка КАК ЧИСЛО(2, 1))) > &КооэфТестирование
	...
	Запрос.Параметры.Вставить("КооэфРазработка", ОбщегоНазначенияКлиентСервер.КоэффициентРазработки());
	Запрос.Параметры.Вставить("КооэфТестирование", ОбщегоНазначенияКлиентСервер.КоэффициентТестирования());
...
Функция ПолучитьРасхождениеПоДатам() Экспорт 
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	Релизы.Ссылка КАК Релиз,
	               |	ЗадачиВерсии.Ссылка КАК Задача,
	               |	Задачи.ПлановаяДатаЗавершения КАК ДатаЗавершенияЗадачи,
	               ...
	               |	И Задачи.ПлановаяДатаЗавершения < Релизы.ДатаРелиза";
	
	Возврат Запрос.Выполнить().Выгрузить();
КонецФункции

Функция ПолучитьРасхождениеПоДатамКоличество() Экспорт 
	Возврат ПолучитьРасхождениеПоДатам().Количество();	
КонецФункции
...
Функция КоличествоКонфликтовПревышениеПоВерсиям() Экспорт 
	Возврат ПолучитьПревышениеПоВерсиям().Количество();
КонецФункции

Функция ПолучитьЗадачиСКоммитамиПослеРелиза() Экспорт 
	Запрос = Новый Запрос;
	
	Запрос.Текст = "ВЫБРАТЬ
	               |	Задачи.Ссылка КАК Задача,
	               |	Релизы.РелизВыпущен
	               |		ИЛИ Релизы.РелизСобран КАК РелизВыпущенСобран,
	               |	Релизы.ДатаРелиза КАК ДатаОкончания,
	               |	Релизы.ДатаНачала КАК ДатаНачала,
	               |	Задачи.Код КАК НомерЗадачи
	               |ПОМЕСТИТЬ ЗадачиСВыпущенымРезизом
	               |ИЗ
	               |	Справочник.Задачи.Версии КАК ЗадачиВерсии
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Релизы КАК Релизы
	               |		ПО ЗадачиВерсии.Версия = Релизы.Ссылка
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ЗадачиСВыпущенымРезизом.Задача КАК Задача
	               |ПОМЕСТИТЬ ЗадачиСРелизамиОдногоСостояния
	               |ИЗ
	               |	ЗадачиСВыпущенымРезизом КАК ЗадачиСВыпущенымРезизом
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ЗадачиСВыпущенымРезизом.Задача
	               |
	               |ИМЕЮЩИЕ
	               |	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЗадачиСВыпущенымРезизом.РелизВыпущенСобран) = 1
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ЗадачиСВыпущенымРезизом.Задача КАК Задача,
	               |	МАКСИМУМ(ЗадачиСВыпущенымРезизом.ДатаОкончания) КАК ДатаОкончания,
	               |	ЗадачиСВыпущенымРезизом.НомерЗадачи КАК НомерЗадачи
	               |ПОМЕСТИТЬ НужныеЗадачи
	               |ИЗ
	               |	ЗадачиСРелизамиОдногоСостояния КАК ЗадачиСРелизамиОдногоСостояния
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ЗадачиСВыпущенымРезизом КАК ЗадачиСВыпущенымРезизом
	               |		ПО ЗадачиСРелизамиОдногоСостояния.Задача = ЗадачиСВыпущенымРезизом.Задача
	               |ГДЕ
	               |	ЗадачиСВыпущенымРезизом.РелизВыпущенСобран
	               |	И ЗадачиСВыпущенымРезизом.ДатаОкончания <> ДАТАВРЕМЯ(1, 1, 1)
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ЗадачиСВыпущенымРезизом.Задача,
	               |	ЗадачиСВыпущенымРезизом.НомерЗадачи
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	НужныеЗадачи.Задача КАК Задача,
	               |	НужныеЗадачи.ДатаОкончания КАК ДатаОкончанияВерсииВЗадаче,
	               |	МАКСИМУМ(БФТ_ИзмененияРепозитория.Период) КАК ДатаКоммита,
	               |	НужныеЗадачи.НомерЗадачи КАК НомерЗадачи
	               |ПОМЕСТИТЬ ГотовыеДанные
	               |ИЗ
	               |	НужныеЗадачи КАК НужныеЗадачи
	               ...
	               |				ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.БФТ_ИзмененияРепозитория КАК БФТ_ИзмененияРепозитория
	               |				ПО БФТ_НаборыИзмененийНаборИзменений.НомерРевизии = БФТ_ИзмененияРепозитория.НомерРевизии
	               |					И БФТ_НаборыИзмененийНаборИзменений.ИзменениеКонфигурации = БФТ_ИзмененияРепозитория.ИзменениеКонфигурации
	               |			ПО БФТ_НаборыИзменений.Ссылка = БФТ_НаборыИзмененийНаборИзменений.Ссылка
	               |		ПО НужныеЗадачи.НомерЗадачи = БФТ_НаборыИзменений.Код
	               |ГДЕ
	               |	БФТ_ИзмененияРепозитория.Период > НужныеЗадачи.ДатаОкончания
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	НужныеЗадачи.Задача,
	               |	НужныеЗадачи.ДатаОкончания,
	               |	НужныеЗадачи.НомерЗадачи
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ГотовыеДанные.Задача КАК Задача,
	               |	ГотовыеДанные.ДатаОкончанияВерсииВЗадаче КАК ДатаОкончанияВерсииВЗадаче,
	               |	ГотовыеДанные.ДатаКоммита КАК ДатаКоммита,
	               |	ГотовыеДанные.НомерЗадачи КАК НомерЗадачи,
	               |	Релизы.РелизВыпущен КАК РелизВыпущен,
	               |	Релизы.РелизСобран КАК РелизСобран,
	               |	Релизы.Ссылка КАК Релиз
	               |ИЗ
	               |	ГотовыеДанные КАК ГотовыеДанные
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Релизы КАК Релизы
	               |		ПО (ГотовыеДанные.ДатаКоммита МЕЖДУ Релизы.ДатаНачала И Релизы.ДатаРелиза)
	               |ИТОГИ ПО
	               |	Релиз";
