...
Функция ПолучитьЗадачиСКоммитамиПослеРелиза() Экспорт 
	...
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	БФТ_НаборыИзменений.Код КАК Код,
		...
		|	Задачи.ДатаКоммита КАК ДатаКоммита
		|ИЗ
		|	Задачи КАК Задачи
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Задачи.Версии КАК ЗадачиВерсии
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Релизы КАК Релизы
		|			ПО ЗадачиВерсии.Версия = Релизы.Ссылка
		|		ПО Задачи.Задача = ЗадачиВерсии.Ссылка
		|			И (Задачи.ДатаВыпускаПоследнегоРелиза = Релизы.ДатаРелиза)
		|ГДЕ
		|	Задачи.РелизСобран
		|	И НЕ Задачи.РелизВыпущен
		|	И Задачи.Релиз.ДатаНачала >= Релизы.ДатаНачала
		|ИТОГИ ПО
		|	Задача";
