...
Функция ПолучитьЗадачиСКоммитамиПослеРелиза() Экспорт 
	...
	Запрос.Текст = 
		...
		|		ПО (БФТ_ИзмененияРепозитория.Период МЕЖДУ Релизы.ДатаНачала И Релизы.ДатаРелиза)
		|ГДЕ
		|	Релизы.ДатаНачала <> ДАТАВРЕМЯ(1, 1, 1)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	Релизы.РелизВыпущен КАК РелизВыпущен,
		|	Релизы.ДатаРелиза КАК ДатаРелиза,
		|	ФактДанныеКоммита.Период КАК Период,
		|	ФактДанныеКоммита.НаборИзмененийКод КАК НаборИзмененийКод
		|ПОМЕСТИТЬ Коммиты
		|ИЗ
		|	ФактДанныеКоммита КАК ФактДанныеКоммита
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Релизы КАК Релизы
		|		ПО (НЕ ФактДанныеКоммита.РелизСобран
		|					И ФактДанныеКоммита.Релиз = Релизы.Ссылка
		|				ИЛИ ФактДанныеКоммита.ДатаРелиза < Релизы.ДатаРелиза)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА Задачи.Родитель = ЗНАЧЕНИЕ(Справочник.Задачи.ПустаяСсылка)
		|			ТОГДА Задачи.Ссылка
		|		ИНАЧЕ Задачи.Родитель
		|	КОНЕЦ КАК Задача,
		|	Коммиты.Период КАК ДатаКоммита,
		|	Коммиты.Релиз КАК Релиз,
		|	Коммиты.РелизСобран КАК РелизСобран,
		|	Коммиты.РелизВыпущен КАК РелизВыпущен,
		|	МАКСИМУМ(Релизы.ДатаРелиза) КАК ДатаВыпускаПоследнегоРелиза
		|ПОМЕСТИТЬ ДанныеКоммитов
		|ИЗ
		|	Коммиты КАК Коммиты
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Задачи КАК Задачи
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Задачи.Версии КАК ЗадачиВерсии
		|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Релизы КАК Релизы
		|				ПО ЗадачиВерсии.Версия = Релизы.Ссылка
		|			ПО Задачи.Ссылка = ЗадачиВерсии.Ссылка
		|		ПО Коммиты.НаборИзмененийКод = Задачи.Код
		|
		|СГРУППИРОВАТЬ ПО
		|	ВЫБОР
		|		КОГДА Задачи.Родитель = ЗНАЧЕНИЕ(Справочник.Задачи.ПустаяСсылка)
		...
		|	Коммиты.Релиз,
		|	Коммиты.РелизСобран,
		|	Коммиты.РелизВыпущен
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ДанныеКоммитов.Задача КАК Задача,
		|	Релизы.Ссылка КАК РелизВЗадаче,
		|	ДанныеКоммитов.ДатаКоммита КАК ДатаКоммита,
		|	ДанныеКоммитов.Релиз КАК РелизКоммита
		|ИЗ
		|	ДанныеКоммитов КАК ДанныеКоммитов
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Задачи.Версии КАК ЗадачиВерсии
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Релизы КАК Релизы
		|			ПО ЗадачиВерсии.Версия = Релизы.Ссылка
		|		ПО ДанныеКоммитов.Задача = ЗадачиВерсии.Ссылка
		|			И (ДанныеКоммитов.ДатаВыпускаПоследнегоРелиза = Релизы.ДатаРелиза)
		|ГДЕ
		|	ДанныеКоммитов.РелизСобран
		|	И НЕ ДанныеКоммитов.РелизВыпущен
		|	И ДанныеКоммитов.Релиз.ДатаНачала > Релизы.ДатаНачала
		|ИТОГИ ПО
		|	Задача";
