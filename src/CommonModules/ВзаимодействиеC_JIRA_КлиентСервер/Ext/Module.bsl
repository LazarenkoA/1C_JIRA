...
Процедура УдалитьФайлыКлиентСервер(ФайлОтвет) 
	#Если Клиент Тогда
		ОО = Новый ОписаниеОповещения("ЗавершениеУдалениеФайлов", ЭтотОбъект);
		НачатьУдалениеФайлов(ОО, ФайлОтвет);
	#Иначе
...
Процедура УстановитьСтатусЗадаче(НомерЗадачи, ИмяСтатуса, ДопДанные) Экспорт 
	...
	Если ВРег(ИмяСтатуса) = "ПРИНЯТ" И ДопДанные <> Неопределено Тогда
		СтруктураДанных.Вставить("fields", Новый Структура("timetracking,duedate,customfield_12500"));	// Поля необходимые при переходе в принят	
...
Процедура ВыполнитьPUTЗапрос(URL, ДанныеJSON)
  ...
  Заголовки.Вставить("Cookie", Куки); 
	...
	Попытка
		...
		УдалитьФайлыКлиентСервер(ФайлОтвет);
	Исключение
		УдалитьФайлыКлиентСервер(ФайлОтвет);
		ВызватьИсключение;
	КонецПопытки;	
...
Процедура ЗавершениеУдалениеФайлов(ДополнительныеПараметры) Экспорт 
