Функция ПолучитьЭффективностиСотрудников(ДатаНач, ДатаКон) Экспорт 
	Запрос = Новый Запрос;
	...
	Запрос.УстановитьПараметр("КонецСобытия", ДатаКон);    
	Запрос.УстановитьПараметр("НачалоСобытия", ДатаНач);
	...
	Для Каждого Стр Из РезультатЗапроса Цикл
		Стр.Эффективность = Стр.ОтношениеКоличеств + (Стр.ОтношениеКоличеств * (Стр.КолВоЗадачВышеСреднего / Стр.КоличествоПоСотруднику));
	КонецЦикла;
	...
	Для Каждого Стр Из РезультатЗапроса Цикл
		...
		КоличествоВозвратов = КоличествоВозвратов * 0.75; // Коэффициент взят с потолка (3/4).
		КоэфВозвратов = КоличествоВозвратов / Стр.КоличествоПоСотруднику;
		Если КоэфВозвратов = 0 Тогда
			Продолжить;	
		КонецЕсли;
		
		Стр.Эффективность = Стр.Эффективность - (Стр.Эффективность * КоэфВозвратов);
	КонецЦикла;
...
Функция КварталВДату(НомерКвартала) Экспорт 
	Возврат Дата(Год(ТекущаяДата()), (НомерКвартала * 3) - 2, 1);
КонецФункции
