...
Процедура АнализИзмененийХранилища1С() Экспорт 
	ПользовательХранилища = ОбщегоНазначенияСервер.ПолучитьЗначениеКонстанты("БФТ_ПользовательХранилища");
	АдресХранилища = ОбщегоНазначенияСервер.ПолучитьЗначениеКонстанты("БФТ_АдресХранилища");
	ПарольХранилища = ОбщегоНазначенияСервер.ПолучитьЗначениеКонстанты("БФТ_ПарольХранилища");
...
//
// Параметры:
//  МетодСсылка - СправочникСсылка - ссылка на элемент справочника.
//
// Возвращаемое значение: 
//  СправочникСсылка - ссылка на элемент справочника.
//
&НаСервере
Функция ПолучитьНастройки(МетодСсылка) Экспорт 
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	БФТ_ПараметрыВыполненияКоманд.РежимВводаПараметра,
	|	БФТ_ПараметрыВыполненияКоманд.ЗначениеПараметра,
	|	БФТ_ПараметрыВыполненияКоманд.ИсполняемыйФайл,
	|	БФТ_ПараметрыВыполненияКоманд.ПорядковыйНомерПараметра
	|ИЗ
	|	РегистрСведений.БФТ_ПользовательскиеНастройкиПодключенияКРепозиторию КАК БФТ_ПараметрыВыполненияКоманд
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.БФТ_МетодыРаботыСРепозиторием КАК БФТ_МетодыРаботыСРепозиторием
	|		ПО БФТ_ПараметрыВыполненияКоманд.Объект = БФТ_МетодыРаботыСРепозиторием.Ссылка
	|ГДЕ
	|	(БФТ_ПараметрыВыполненияКоманд.Пользователь = &Пользователь
	|				И НЕ ЕСТЬNULL(БФТ_МетодыРаботыСРепозиторием.СерверныеНастройки, ЛОЖЬ)
	|			ИЛИ ЕСТЬNULL(БФТ_МетодыРаботыСРепозиторием.СерверныеНастройки, ЛОЖЬ)
	|				И БФТ_ПараметрыВыполненияКоманд.Пользователь = &ПустойGuid)
	|	И БФТ_ПараметрыВыполненияКоманд.Объект = &Объект";
	
	Запрос.УстановитьПараметр("Объект", МетодСсылка);
	Запрос.УстановитьПараметр("Пользователь", ПользователиИнформационнойБазы.ТекущийПользователь().УникальныйИдентификатор);
	Запрос.УстановитьПараметр("ПустойGuid", ОбщегоНазначенияКлиентСервер.ПустоеЗначениеТипа(Тип("УникальныйИдентификатор")));
...
Процедура СоздатьЭлементыФормыСписка(Форма, ГруппаРодитель = Неопределено) Экспорт 
	...
	Если Форма.Команды.Найти(ИмяКоманды) = Неопределено Тогда
		...
		КомандаФормы.Действие = "ПолучитьИзмененияРепозитория";
...
Процедура СоздатьЗаписи(ДанныеХранилища)
	// Получаем из SVN последнюю загруженую ревизию.
	// Все записи которые меньше или равны последней загруженной, статус должны иметь "Загруженно".
	
	// ВерсияИзРепозитория = РегистрыСведений.БФТ_ОчередьВыгрузкиРевизийКонфигурации1С.ПолучитьПоследнююЗагруженнуюРевизию();
	Для Каждого Данные Из ДанныеХранилища Цикл
		Запись = РегистрыСведений.БФТ_ОчередьВыгрузкиРевизийКонфигурации1С.СоздатьМенеджерЗаписи();
		Данные.Свойство("Пользователь", Запись.АвторИзменений);
		Данные.Свойство("Версия", Запись.НомерРевизии);
		Данные.Свойство("Комментарий", Запись.Комментарий);
		Запись.Статус = Перечисления.БФТ_СтатусИзмененийРепозитория.НеЗагружено;
		// Запись.Статус = ?(Запись.НомерРевизии <=.
		// ВерсияИзРепозитория, Перечисления.БФТ_СтатусИзмененийРепозитория.Загружено, Перечисления.БФТ_СтатусИзмененийРепозитория.НеЗагружено);
		Запись.ДатаИзменений = СтроковыеФункцииКлиентСервер.СтрокаВДату(Данные.Дата);
