...
Процедура АнализИзмененийХранилища1С() Экспорт 
	...
	Попытка
		...
		Попытка
			...
			Отбор = Новый Структура("ИзменениеКонфигурации", Истина);
			Выборка = РегистрыСведений.БФТ_ИзмененияРепозитория.Выбрать(?(ПоследняяЗапись = Неопределено, Дата(1,1,1), ПоследняяЗапись.Период),, Отбор);
...
Процедура РазобратьФайл(Адрес, ДокументDOM) Экспорт 
	...
	Попытка
		...
		Попытка
			...
			Отбор = Новый Структура("ИзменениеКонфигурации", Ложь);
			Выборка = РегистрыСведений.БФТ_ИзмененияРепозитория.Выбрать(?(ПоследняяЗапись = Неопределено, Дата(1,1,1), ПоследняяЗапись.Период),, Отбор);
			Пока Выборка.Следующий() Цикл 
				ПривязатьКЗадачам(Выборка.Комментарий, Выборка.НомерРевизии);
			КонецЦикла;
