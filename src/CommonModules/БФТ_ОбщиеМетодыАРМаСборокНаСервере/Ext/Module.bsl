...
Процедура АнализИзмененийХранилища1С() Экспорт 
	...
	Попытка
		...
		ДанныеОтчета = ПарсОтчета(ФайлОтчета);
		НачатьТранзакцию();
		Попытка
			СоздатьЗаписи(ДанныеОтчета);
...
Процедура ПолучитьАктуальнуюРевизиюИзSVN() Экспорт
...
&НаСервере
Процедура ПоместитьНовуюВерсиюВSVN() Экспорт
...
Процедура СоздатьЗаписьИзмененныхОбъектов(ЗаписьИзмРепы, Данные)
	...
	Попытка
		...
		Для а = 0 По Данные.Добавлены.Количество()-1 Цикл
			НовСтр = Набор.Добавить();
			НовСтр.НомерРевизии = ЗаписьИзмРепы.НомерРевизии;
			НовСтр.ИзменениеКонфигурации = ЗаписьИзмРепы.ИзменениеКонфигурации;
			НовСтр.ПорядковыйНомер = а;
			НовСтр.Тип = Перечисления.ТипИзмененийОбъекта.ОбъектДобавлен;
			НовСтр.Объект = Данные.Добавлены[а];
		КонецЦикла;
...
&НаСервере
Функция СформироватьСписокИзменений(ЭлементDOM)
	...
	Пока Узел <> Неопределено Цикл
		...
		ИначеЕсли Действие = Перечисления.ТипИзмененийОбъекта.ОбъектУдален Тогда
			Изменения.Удалены.Добавить(Узел.ТекстовоеСодержимое);	
		КонецЕсли;
