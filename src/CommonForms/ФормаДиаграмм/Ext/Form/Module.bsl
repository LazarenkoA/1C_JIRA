...
Процедура ВывестиДиаграмму(МассивЗадач)
	...
	Пока ВыборкаИсполнитель.Следующий() Цикл
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			ФактКалендарныхЧасовВРаботе = (ВыборкаДетальныеЗаписи.ФактЧасовВРаботе / 8) * 24; // ФактЧасовВРаботе - это в раб. часах, переводим в календарные, получается +-
			ФактическийКонец = Интервал.Начало + (ФактКалендарныхЧасовВРаботе * 60*60);
