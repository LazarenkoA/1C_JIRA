...
Функция ПроверкаСтатусаРегЗаданияНаСервере(РучнойЗапускРегЗадания)
	...
	Если РЗ <> Неопределено И РЗ.ПоследнееЗадание <> Неопределено Тогда
		...
		Иначе
			КонецПоследнегоЗадания = ?(РЗ.ПоследнееЗадание.Конец <> Неопределено, РЗ.ПоследнееЗадание.Конец, 0);
			ПериодПовтораВТечениеДня = ?(РЗ.Расписание.ПериодПовтораВТечениеДня <> Неопределено, РЗ.Расписание.ПериодПовтораВТечениеДня, 0);
			
			Попытка
				СлВыполнение = КонецПоследнегоЗадания + ПериодПовтораВТечениеДня;
			Исключение
				ЗаписьЖурналаРегистрации("КонецПоследнегоЗадания", УровеньЖурналаРегистрации.Ошибка,,, Строка(КонецПоследнегоЗадания));
				ЗаписьЖурналаРегистрации("ПериодПовтораВТечениеДня", УровеньЖурналаРегистрации.Ошибка,,, Строка(ПериодПовтораВТечениеДня));
				ВызватьИсключение;
			КонецПопытки;
			
			РазницаМинут = (СлВыполнение - ТекущаяДатаСеанса()) / 60;
			Ответ = СтрШаблон("Запуск обновления через %1 мин.", Формат(РазницаМинут, "ЧДЦ=1; ЧГ="));
		КонецЕсли;
	КонецЕсли;
