...
&НаСервереБезКонтекста
Процедура ЗаполнитьДерево(Дерево, Исполнитель)
...
Процедура ОбновитьДанныеКоманды(ОбновлятьЭффективность)
	Для Каждого Эл Из ЭтаФорма.Элементы Цикл
		...
		Если ОбновлятьЭффективность И СтрНачинаетсяС(Эл.Имя, "ПрогрессЭффективность") Тогда 
			...
			ДатаНач = ОбщегоНазначенияСерверПовтИсп.КварталВДату(Кварталы);
			ДатаКон = КонецКвартала(ДатаНач);
			ЭтаФорма[Эл.ПутьКДанным] = ПолучитьПроцентЭффективности(Пользователь, ДатаНач, ДатаКон);   
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
&НаСервереБезКонтекста
Функция ПолучитьПроцентЭффективности(Сотрудник, ДатаНач, ДатаКон)
	...
	Результат = ОбщегоНазначенияСерверПовтИсп.ПолучитьЭффективностиСотрудников(ДатаНач, ДатаКон);
...
&НаКлиенте
Процедура КварталыПриИзменении(Элемент)
	ОбновитьДанныеКоманды(Истина);
КонецПроцедуры
